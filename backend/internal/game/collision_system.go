...
// GetNearbyObjects возвращает объекты рядом с позицией (публичный метод)
func (cd *ColliderDispatcher) GetNearbyObjects(pos Vector3, radius float64) []*CollidableObject {
	return cd.spatialGrid.GetNearbyObjects(pos, radius)
}
